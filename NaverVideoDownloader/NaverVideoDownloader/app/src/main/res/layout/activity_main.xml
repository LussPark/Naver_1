...
class MainActivity : AppCompatActivity() {
    ...
    // 자동 재생 완료 신호가 중복으로 여러 번 오는 것을 방지
    ...
    inner class JsBridge {
        ...
        fun onVideoEnded() {
            runOnUiThread {
                if (autoDownloadTriggered) return@runOnUiThread
                autoDownloadTriggered = true
                appendLog("자동 배속 재생 완료 감지 -> 자동 다운로드 시작")
                downloadBest()
            }
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.webView.webViewClient = object : WebViewClient() {
            override fun shouldInterceptRequest(
                ...
                when {
                    // 1) 재생목록 파일 자체가 잡히는 경우 (가장 이상적인 경우)
    ...
    /** og:title(대표 메타 태그)을 우선 사용하고, 없으면 document.title로 대체해 기사 제목을 가져온다. */
    private fun extractArticleTitle(view: WebView) {
        val script = """
            (function() {
        ...
        view.evaluateJavascript(script) { rawResult ->
            ...
            if (!decoded.isNullOrBlank()) {
                currentArticleTitle = decoded
                appendLog("기사 제목 인식: $decoded")
            }
        }
    }
    ...
    private fun onPlaylistDetected(url: String) {
        if (detectedPlaylists.add(url)) {
            ...
            binding.statusText.text = "상태: 스트림 ${detectedPlaylists.size}개 감지됨 (다운로드 가능)"
    ...
    private fun onSegmentDetected(url: String) {
        ...
        if (!map.containsKey(seq)) {
            ...
            binding.statusText.text = "상태: 세그먼트 ${map.size}개 수집됨 (다운로드 가능)"
            binding.downloadButton.isEnabled = true
        }
    }
    ...
    /**
     * 기사 제목을 파일명으로 사용한다. 제목을 못 가져온 경우 시간 기반 이름으로 대체하고,
     * 같은 이름 파일이 이미 있으면 뒤에 번호를 붙인다.
     */
    private fun buildOutputFileName(extension: String): String {
        val base = if (currentArticleTitle.isNotBlank()) {
